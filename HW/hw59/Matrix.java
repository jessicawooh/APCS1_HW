...
public class Matrix {
	...
	Object[] oldRow = matrix[r];
	matrix[r] = newRow;
	return oldRow;
    }//O(1)

    public Object[] setCol( int c, Object[] newCol ) {
	Object[] oldCol = new Object[matrix.length];
	for (int i = 0; i < matrix.length; i++){
	    matrix[i][c] = newCol[i];	
	    oldCol[i] = matrix[i][c];
	}
	...
	Object[][] temp = new Object[matrix.length][matrix[0].length];
	for (int i = 0; i < matrix.length; i++){
	    for (int j = 0; j < matrix[0].length; j++){
		temp[i][j] = matrix[j][i];
	    }
	...
	System.out.println(clive.isEmpty(1,1));
	...
	Object[] testrow = {"apples", "bananas", "cherries","peaches"};
	    
	//setRow
	//	cade.setRow(0, testrow);
	//	System.out.println(cade);

	//setCol
	cade.setCol(1, testrow);
	System.out.println(cade);
